...
# 속성중에 disabled = True 인 엘리먼트는 사용자 클릭을 막아놓음
...
# Selector (선택자): 브라우저가 요소를 찾을 때 쓰는 CSS 선택자 문법. 예) #login, .btn.primary, div.card > a.title.
# DevTool의 Copy... 종류의 차이
# Copy selector : CSS 선택지를 생성해줌 #main > div:nth-child(3) > a.btn.primary
# Copy JS path : JS 코드 (document, queryselector..)
# Copy Xpath / full Xpath : 
# Xpath : 상대경로 (중요 표지판 같은 역할?)//div[@class='card']//a[@role='button'] - 중요 기준점을 잡고 그 아래부터 기록
#           어디서든 조건에 맞는 구간을 기준으로. 너무 길면 상대적으로 취약하기에 중요부분만 짧게 갖는 게 변경에 더 강하다고 함.
# Full Xpath : full XPath: 문서 루트부터 절대경로(/html/body/.../div[3]/a) - 모든 경로 빠짐없이 기록