# shop_base2_tbl
import pymysql
from dotenv import load_dotenv
import os
# .env 로드
load_dotenv()

def get_connection():
    return pymysql.connect(
        host = os.getenv('DB_HOST'),
        user = os.getenv('DB_USER'),
        password = os.getenv('DB_PASSWORD'),
        database='shopinfo'
    )
import crawlingcoffee
# terminal에서 pip install tqdm  (진행상황을 보여줌)
import tqdm
for page_num in tqdm.tqdm(range(1,47)):
    datas = crawlingcoffee.get_data(page_num)
    with get_connection() as conn:
        with conn.cursor() as cur:
            for data in datas:
                try:
                    sql = 'insert into shop_base2_tbl(shop_area, shop_name, shop_state, shop_addr, shop_phone_number) values(%s,%s,%s,%s,%s)'
                    cur.execute(sql, (data[0],data[1],data[2],data[3],data[4]))
                except pymysql.err.IntegrityError:  #값이 이미 있어서 중복 에러가 생길 가능성
                    sql = '''update shop_base2_tbl
                                set shop_state=%s, shop_addr=%s,shop_phone_number=%s
                            where shop_area=%s and shop_name=%s
                    '''
                    cur.execute(sql, (data[2],data[3],data[4],data[0],data[1]))
            conn.commit()

            # 먼저 insert를 시도하고 만약 중복이면 update할 것